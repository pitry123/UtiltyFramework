...
namespace http
{
	class DLL_EXPORT curl_http_client : public core::http::http_client_interface
	{
	public:
		virtual ~curl_http_client() = default;
		static bool create(const char* content_type, core::http::http_client_interface** client);
		...
		enum curl_code
		{
			...
			/* Note: CURLE_OUT_OF_MEMORY may sometimes indicate a conversion error
					 instead of a memory allocation error if CURL_DOES_CONVERSIONSis defined*/
			...
			CURLE_SSH,						/* 79 - error from the SSH layer,  generic so the error message will be interest when this has happened */
			CURLE_SSL_SHUTDOWN_FAILED,		/* 80 - Failed to shut down the  connection */
