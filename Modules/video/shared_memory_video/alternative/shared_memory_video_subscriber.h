...
namespace video
{
	namespace sources
	{
		...
		class shared_memory_video_subscriber : 
			public utils::ref_count_base<utils::video_source_base<video::sources::shared_memory_video_source>>
		{
		private:
			...
			utils::ref_count_ptr<shared_memory_session_reader> m_reader;

		public:
			shared_memory_video_subscriber(const char* video_name) :
				m_state(core::video::video_state::STOPPED)
			{
				m_reader = utils::make_ref_count_ptr<shared_memory_session_reader>(video_name, 2);

				m_reader->OnBuffer += [this](uint32_t stream_index, core::buffer_interface* buffer)
				{
			...
			virtual void start() override
			{
				...
				if (m_state != core::video::video_state::PLAYING)
				{
					...
					{
						m_reader->start(VIDEO_CHANNEL_ID);
					}
			...
			virtual void stop() override
			{
				...
				{
					m_state = core::video::video_state::STOPPED;
					m_reader->stop(VIDEO_CHANNEL_ID);
				}
			}
