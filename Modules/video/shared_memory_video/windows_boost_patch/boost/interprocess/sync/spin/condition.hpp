...
class spin_condition
{
   ...
   bool notify(boost::uint32_t command, const boost::posix_time::ptime &abs_time);
};
...
inline bool spin_condition::notify_one(const boost::posix_time::ptime &abs_time)
{
	return this->notify(NOTIFY_ONE, abs_time);
}

inline bool spin_condition::notify_all(const boost::posix_time::ptime &abs_time)
{
...
inline bool spin_condition::notify(boost::uint32_t command, const boost::posix_time::ptime &abs_time)
{
	//This mutex guarantees that no other thread can enter to the
	//do_timed_wait method logic, so that thread count will be
	//constant until the function writes a NOTIFY_ALL command.
	//It also guarantees that no other notification can be signaled
	//on this spin_condition before this one ends
	if (m_enter_mut.timed_lock(abs_time) == false)
		return false;
