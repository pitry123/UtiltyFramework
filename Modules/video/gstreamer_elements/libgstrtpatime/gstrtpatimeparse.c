...
#define LOCAL_CLOCK_SIMULATION TRUE
...
handle_buffer (GstRtpatimeParse * self, GstBuffer * buf)
{
  ...
  if ((!gst_rtp_buffer_get_extension_data(&rtp, &bits, (gpointer)& data, &wordlen)) ||
	  (bits != EXTENSION_ID || wordlen != EXTENSION_SIZE))
  {
	  if (!LOCAL_CLOCK_SIMULATION)
		  goto out;
