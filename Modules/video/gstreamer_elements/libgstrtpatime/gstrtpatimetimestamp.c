...
GST_DEBUG_CATEGORY_STATIC (rtpatimetimestamp_debug);
#define GST_CAT_DEFAULT (rtpatimetimestamp_debug)

static GstFlowReturn gst_rtp_atime_timestamp_chain (GstPad * pad,
    GstObject * parent, GstBuffer * buf);
static GstFlowReturn gst_rtp_atime_timestamp_chain_list (GstPad * pad,
...
/*static guint gst_rtp_atime_timestamp_signals[LAST_SIGNAL] = { 0 }; */

G_DEFINE_TYPE (GstRtpatimeTimestamp, gst_rtp_atime_timestamp, GST_TYPE_ELEMENT);

static void
gst_rtp_atime_timestamp_get_property (GObject * object,
    guint prop_id, GValue * value, GParamSpec * pspec)
{
  GstRtpatimeTimestamp *self = GST_RTP_atime_TIMESTAMP (object);
...
static void
gst_rtp_atime_timestamp_set_property (GObject * object,
...
gst_rtp_atime_timestamp_change_state (GstElement * element,
    GstStateChange transition)
{
  GstRtpatimeTimestamp *self = GST_RTP_atime_TIMESTAMP (element);
  ...
  ret = GST_ELEMENT_CLASS (gst_rtp_atime_timestamp_parent_class)->change_state
      (element, transition);
...
static void
gst_rtp_atime_timestamp_finalize (GObject * object)
{
  GstRtpatimeTimestamp *self = GST_RTP_atime_TIMESTAMP (object);
  ...
  G_OBJECT_CLASS (gst_rtp_atime_timestamp_parent_class)->finalize (object);
}

static void
gst_rtp_atime_timestamp_class_init (GstRtpatimeTimestampClass * klass)
{
  ...
  gobject_class->get_property = gst_rtp_atime_timestamp_get_property;
  gobject_class->set_property = gst_rtp_atime_timestamp_set_property;
  gobject_class->finalize = gst_rtp_atime_timestamp_finalize;
  ...
  g_object_class_install_property (gobject_class, PROP_SET_E_BIT,
      g_param_spec_boolean ("set-e-bit", "Set 'E' bit",
          "If the element should set the 'E' bit as defined in the atime RTP "
  ...
  gstelement_class->change_state =
      GST_DEBUG_FUNCPTR (gst_rtp_atime_timestamp_change_state);

  GST_DEBUG_CATEGORY_INIT (rtpatimetimestamp_debug, "rtpatimetimestamp",
      0, "atime NTP timestamps RTP extension");
}

static GstFlowReturn handle_and_push_buffer (GstRtpatimeTimestamp * self,
    GstBuffer * buf, gboolean end_contiguous);
static GstFlowReturn handle_and_push_buffer_list (GstRtpatimeTimestamp * self,
    GstBufferList * list, gboolean end_contiguous);

static gboolean
gst_rtp_atime_timestamp_sink_event (GstPad * pad, GstObject * parent,
    GstEvent * event)
{
  GstRtpatimeTimestamp *self = GST_RTP_atime_TIMESTAMP (parent);
...
static void
gst_rtp_atime_timestamp_init (GstRtpatimeTimestamp * self)
{
  self->sinkpad =
      gst_pad_new_from_static_template (&sink_template_factory, "sink");
  gst_pad_set_chain_function (self->sinkpad, gst_rtp_atime_timestamp_chain);
  gst_pad_set_chain_list_function (self->sinkpad,
      gst_rtp_atime_timestamp_chain_list);
  gst_pad_set_event_function (self->sinkpad,
      gst_rtp_atime_timestamp_sink_event);
...
static gboolean
handle_buffer (GstRtpatimeTimestamp * self, GstBuffer * buf,
    gboolean end_contiguous)
{
  ...
  ATimeMeta* atime_meta = gst_buffer_get_atime_meta(buf);
  if (atime_meta != NULL)
  {
	  // If we've already got meteadata containing
	  // a timestamp, use it
	  time = atime_meta->absoluteTime;
  }
  else
  {
	  // No metadata - Generate timestamp
...
handle_and_push_buffer (GstRtpatimeTimestamp * self, GstBuffer * buf,
    gboolean end_contiguous)
{
...
static GstFlowReturn
gst_rtp_atime_timestamp_chain (GstPad * pad, GstObject * parent,
    GstBuffer * buf)
{
  GstRtpatimeTimestamp *self = GST_RTP_atime_TIMESTAMP (parent);
...
/* @buf: (transfer all) */
static GstFlowReturn
handle_and_push_buffer_list (GstRtpatimeTimestamp * self,
    GstBufferList * list, gboolean end_contiguous)
{
  GstBuffer *buf;
...
static GstFlowReturn
gst_rtp_atime_timestamp_chain_list (GstPad * pad, GstObject * parent,
    GstBufferList * list)
{
  GstRtpatimeTimestamp *self = GST_RTP_atime_TIMESTAMP (parent);
  GstFlowReturn result = GST_FLOW_OK;
